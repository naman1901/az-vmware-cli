...
def privatecloud_create(
    ...
    cluster_size,
    network_block,
    circuit_primary_subnet=None,
    circuit_secondary_subnet=None,
    internet=None,
    vcenter_password=None,
    nsxt_password=None,
    tags=[],
):
    from azext_vmware.vendored_sdks.models import (
        PrivateCloud,
    ...
    if circuit_primary_subnet is not None or circuit_secondary_subnet is not None:
        circuit = Circuit(
            primary_subnet=circuit_primary_subnet,
            secondary_subnet=circuit_secondary_subnet,
        )
    else:
        circuit = None
    clusterProps = DefaultClusterProperties(cluster_size=cluster_size)
    cloudProps = PrivateCloudProperties(
        circuit=circuit, cluster=clusterProps, network_block=network_block
    )
    cloud = PrivateCloud(
        location=location, sku=Sku(name=sku), properties=cloudProps, tags=tags
    )
...
def privatecloud_update(
    cmd, client: AVSClient, resource_group_name, name, cluster_size=None, internet=None
):
...
def privatecloud_listadmincredentials(
    cmd, client: AVSClient, resource_group_name, private_cloud
):
    return client.private_clouds.list_admin_credentials(
        resource_group_name=resource_group_name, private_cloud_name=private_cloud
    )


def privatecloud_addidentitysource(
    cmd,
    client: AVSClient,
    resource_group_name,
    name,
    ...
    return client.private_clouds.update(
...
def privatecloud_deleteidentitysource(
    cmd, client: AVSClient, resource_group_name, name, private_cloud, alias, domain
):
    from azext_vmware.vendored_sdks.models import IdentitySource
    ...
    found = next(
        (
            ids
            for ids in pc.properties.identity_sources
            if ids.name == name and ids.alias == alias and ids.domain == domain
        ),
        None,
    )
    if found:
...
def privatecloud_addauthorization(
    cmd, client: AVSClient, resource_group_name, private_cloud, name
):
...
def privatecloud_deleteauthorization(
    cmd, client: AVSClient, resource_group_name, private_cloud, name
):
    from azext_vmware.vendored_sdks.models import ExpressRouteAuthorization
    ...
    pc = client.private_clouds.get(resource_group_name, private_cloud)
    found = next(
        (auth for auth in pc.properties.circuit.authorizations if auth.name == name),
        None,
    )
    if found:
...
def cluster_create(
    cmd, client: AVSClient, resource_group_name, name, private_cloud, size, tags=[]
):
    ...
    return client.clusters.create_or_update(
        resource_group_name=resource_group_name,
        private_cloud_name=private_cloud,
        cluster_name=name,
        cluster=cluster,
