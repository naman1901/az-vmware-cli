...
def load_arguments(self, _):
    ...
    with self.argument_context("vmware private-cloud") as c:
        c.argument(
            ...
            help="A /30 subnet for the primary circuit in the Express Route to configure routing between your network and Microsoft's Enterprise edge (MSEEs) routers.",
        )
        c.argument(
            "circuit_secondary_subnet",
            help="A /30 subnet for the secondary circuit in the Express Route to configure routing between your network and Microsoft's Enterprise edge (MSEEs) routers.",
        )
        c.argument(
            "cluster_size",
        ...
        c.argument(
            "network_block",
            help="A subnet at least of size /22. Make sure the CIDR format is conformed to (A.B.C.D/X) where A,B,C,D are between 0 and 255, and X is between 0 and 22.",
        )
    ...
    with self.argument_context("vmware cluster") as c:
        ...
        c.argument(
            "size",
            help="Number of hosts for the cluster. Minimum of 3 and a maximum of 16.",
        )

    with self.argument_context("vmware private-cloud create") as c:
        c.argument(
            "name", options_list=["--name", "-n"], help="Name of the private cloud."
        )
        c.argument("sku", help="The product SKU.")
        c.argument(
            "internet",
            help='Connectivity to internet. Specify "Enabled" or "Disabled".',
        )
    ...
    with self.argument_context("vmware private-cloud addauthorization") as c:
        c.argument(
            "name", options_list=["--name", "-n"], help="Name of the authorization."
        )
    ...
    with self.argument_context("vmware private-cloud addidentitysource") as c:
        ...
        c.argument(
            "ssl",
            help='Protect LDAP communication using SSL certificate (LDAPS). Specify "Enabled" or "Disabled".',
        )
        c.argument(
            "username",
